...
public:
    ...
    Udpserver(uint16_t port, func_t fun) : _port(port), _func(fun), _isrunning(false)
    {
    }
    void InitServer()
    {
    ...
    void Start()
    {
        ...
        while (1)
        {
            ...
            if (rnum > 0)
            {
                ...
                ssize_t snum = sendto(_fd, response.c_str(), response.size(), 0, (struct sockaddr *)&src, len);
            }
        }
        _isrunning = false;
    }
